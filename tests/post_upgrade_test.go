		...
		Context("running virtual machine with multiple interfaces", func() {
			...
			BeforeAll(func() {
				...
				DeferCleanup(func() {
					...
					_ = deleteNetAttachDef(utils.NamespaceTestDefault, testNetAttachDef.Name)
				})
				...
				DeferCleanup(func() {
					var getErr error
					By("Deleting VM")
					_ = kubevirt.Client().VirtualMachineInstance(testVMI.Namespace).Delete(context.Background(), testVMI.Name, &k8smetav1.DeleteOptions{})
					Eventually(func() error {
						_, getErr = kubevirt.Client().VirtualMachineInstance(testVMI.Namespace).Get(context.Background(), testVMI.Name, &k8smetav1.GetOptions{})
			...
			When("kubevirt updates", func() {
				...
				It("VM should migrate successfully (following 'Migrate' workloads-update-strategy)", func() {
					...
					assertVMIUpdated(testVMI.Namespace, testVMI.Name, migrationTrials)
