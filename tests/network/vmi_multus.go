...
var _ = Describe("[Serial]SRIOV", func() {
	...
	Context("VirtualMachineInstance with sriov plugin interface", func() {
		...
		Context("migration", func() {
			...
			const hotplugSRIOVDeviceTimeout = 30
...
func expectInterfaceToExistByMac(virtClient kubecli.KubevirtClient, vmi *v1.VirtualMachineInstance, macAddress string, timeout int, expectError string) {
	...
	}, timeout, 5*time.Second).Should(Succeed(), expectError)
}
