...
var _ = Describe("FilterDomainInterfaceByName", func() {
	...
	DescribeTable(
		...
		func(ifaceNames []string, domainIfaces []vapi.Interface, expectedIfaces map[string]vapi.Interface) {
			Expect(network.FilterDomainInterfaceByName(ifaceNames, domainIfaces, network.SanitizeDomainDeviceIfaceAliasName)).To(Equal(expectedIfaces))
		},
		Entry(
			"no iface names",
			[]string{},
		...
		Entry(
			...
			map[string]vapi.Interface{
				net1: {Alias: vapi.NewUserDefinedAlias(net1)},
			},
		),
		Entry(
			...
			map[string]vapi.Interface{
				net1: {Alias: vapi.NewUserDefinedAlias(net1)},
				net2: {Alias: vapi.NewUserDefinedAlias(net2)},
			},
		),
	)
})
