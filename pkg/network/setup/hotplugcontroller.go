...
func FilterDomainInterfaceByName(ifaceNames []string, domainIfaces []api.Interface, sanitizeIfaceNameFn func(api.Interface) string) map[string]api.Interface {
	...
	domainIfacesByName := map[string]api.Interface{}
	for _, ifaceName := range ifaceNames {
		if iface, exist := domainIfacesBySanitizedAlias[ifaceName]; exist {
			domainIfacesByName[ifaceName] = iface
		}
	}
	return domainIfacesByName
}
